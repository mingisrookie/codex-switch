...
- 每次启动在后台检查本仓库最新正式 Release；有新版时显示可关闭的非阻塞横幅，点击一次即可自动下载、校验、替换并重启。新进程必须进入 Tauri Ready 并回传受控 ACK；在此之前失败或超时会尝试恢复并重启旧 EXE。
...
Codex Switch 每次启动只在当前进程内检查一次更新，不安装后台服务、不设置自启动项，也不会在运行期间周期轮询。检查源固定为本仓库最新正式 GitHub Release，draft 和 prerelease 不参与比较；网络失败不会阻塞其他功能。发现新版后点击“立即更新”，应用会下载固定的 Windows x64 EXE、核对 GitHub 资产大小和 SHA-256，退出当前进程后由临时 helper 备份并替换原 EXE，再自动启动新版本；启动失败会恢复并重启旧版本。
...
- 热同步不会为 provider 变化重写已经存在的 live JSONL，关闭 Codex 的切换流程才允许流式原子更新 `session_meta.payload.model_provider`，且不改用户或助手正文；比较历史完整性时忽略这个运行态字段，避免把相同正文误判为冲突。
- 在关闭 Codex、允许替换已有文件的同步中，目标 JSONL 是源文件的严格旧前缀时可用增长后的版本原子替换；较短来源或内容分叉时保留目标当前活动文件，分叉内容按基础文件名加内容哈希保留独立副本，不静默覆盖也不反复嵌套 `-imported-*` 后缀。
...
为避免 Dashboard 被备份 payload 的全量哈希阻塞，列表命令先在 mutation guard 临界区清理 legacy plaintext auth，随即释放锁；之后只扫描可读 manifest 元数据，并只对按 `createdAtMs` 排序后的最近 5 个候选执行 payload 大小/SHA-256 强校验。无效候选会跳过。真正恢复时会再次对用户选择的备份做完整强校验，不能拿列表时的结果代替恢复时检查。

每次受管操作会先清理已经被保留组覆盖的旧备份，再估算本轮 current/shared 快照大小；目标卷必须还能容纳估算值并保留 1 GiB 安全余量，否则在写入任何新备份前停止。操作写入终态记录后自动保留两类目录：最新一次产生备份的操作，以及最近一次成功且包含 current/shared 完整快照的操作；两者相同时通常只留下一个完整回滚组。只有 manifest 与全部 payload 大小/SHA-256 均通过校验、且明确出现在本地操作记录中的旧目录才会被自动删除；无法验证、无法归属、非受管或链接目录保持原样。
...
- 后端用进程内 try-lock + Windows 独占 `%APPDATA%\codex-switch\mutation.lock` 文件句柄串行化保存、验证、切换、同步、删除、恢复和一键更新安装；同一进程或第二个 Codex Switch 进程已有写操作时，新操作会立即拒绝。更新进入退出阶段后锁保持到父进程终止，前端同时双向禁用其他 mutation。
