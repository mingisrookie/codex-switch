...
3. 双击运行；首次使用前请先让 ChatGPT Desktop 完成官方登录。
...
2. 确认 ChatGPT Desktop 已安装，并已正常登录你的官方账号。
3. 在左侧点击 **“保存当前账号态”**，把它作为 README 所说的**官方模式**保存。
4. 在右侧点击 **“配置 API 中转站”**，填写 Base URL、模型名和 API Key；这就是 README 所说的**API 模式**。
5. 点击 **“切换到中转站”**，等待切换进度窗口完成并重新打开 ChatGPT Desktop。
6. 需要恢复官方请求方式时，点击 **“切换到 ChatGPT 账号”**。
...
| 模式 | 适用情况 | 工具做什么 |
| --- | --- | --- |
| 官方模式 | 你要继续使用官方 ChatGPT 请求方式 | 恢复受管请求配置，并移除受管 API 模式配置。 |
| API 模式 | 你要通过自己有权使用的 OpenAI-compatible API 请求模型 | 使用你填写的 Base URL、模型名和 API Key 写入受管请求配置。 |
...
- **官方登录：**工具会读取并核对官方登录状态，但不会替换、写回或让你退出官方登录文件。
...
工具会停止后续操作，并在能安全恢复的地方保留或恢复原配置。不要连续重复点击；先查看切换进度窗口和诊断信息，必要时导出诊断包。
