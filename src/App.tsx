...
function App({ loadDashboard = defaultLoadDashboard }: AppProps) {
  ...
  const [activePage, setActivePage] = useState<'runtime' | 'sessions'>('runtime');
  ...
  async function handleDeleteSessions(ids: string[], confirmUnarchived: boolean) {
    await runAction('删除会话', async () => {
      await deleteManagedSessions(ids, confirmUnarchived);
    });
  }

  async function handleRestoreSessionsVisible(ids: string[]) {
    await runAction('恢复会话可见', async () => {
      await restoreSessionsVisible(ids);
    });
  }
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <button
          className={`topbar-tab ${activePage === 'runtime' ? 'active' : ''}`}
          onClick={() => setActivePage('runtime')}
        >
          运行态
        </button>
        <button
          className={`topbar-tab ${activePage === 'sessions' ? 'active' : ''}`}
      ...
      {activePage === 'runtime' ? (
          ...
          <section className="runtime-grid" aria-label="运行态与会话">
            ...
            <aside className="detail-panel session-panel" aria-label="会话同步">
              ...
              <p className="hint no-indent">
                只合并 SQLite 与 sessions JSONL，不替换登录态文件；已归档会话跳过同步，Codex 正在运行时也可以执行。
              </p>
        ...
        <SessionManagementPage
          inventory={data?.managedSessions ?? {
            currentHome: '',
            sharedHome: '',
            totalCount: 0,
            archivedCount: 0,
