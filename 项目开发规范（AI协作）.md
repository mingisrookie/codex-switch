...
- 请求路由切换是上面快照合同的显式窄例外：`validate target-specific auth/config state + local Relay config -> close -> replan config/session view -> atomic config apply -> verify auth absence/bytes + route -> persist provenance/terminal`。Account 目标必须有官方 `auth_mode = chatgpt`；Relay 目标允许 `auth.json` 缺失或为可解析的非官方状态，并允许 live `config.toml` 缺失。路由 mutation 不得执行 Relay 链接状态或 `/models` 网络探测，不得扫描/复制会话正文、创建 checkpoint、执行 provider slot GC 或写/创建 `auth.json`；配置写后的失败必须把原始 `config.toml` 恢复为原字节或原先的缺失状态，但回滚前必须证明 live config 仍是本次写入字节，检测到外部替换时不得覆盖或删除外部文件。成功后只在 terminal 外排队 coalesced Shadow，不得阻塞用户切换。
...
- Windows 关闭态操作必须基于 ToolHelp PID/PPID 关系证明受管进程树，只把 `ChatGPT.exe` / `OpenAI.Codex.exe` 作为受管根，不得按模糊进程名误杀独立 `codex.exe` CLI。需要自动重启时，关闭前对所有受管根尝试捕获且统一验证唯一 AppUserModelID；成功捕获后可把仅属于内置 allowlist 的 AUMID 以版本化、受限大小 regular-file 记录持久化到应用数据目录，用于下次无运行进程时消除 ChatGPT/Codex 双注册歧义。持久记录损坏、篡改、失效或不再注册必须 fail closed，缺失时只有注册表结果唯一才可使用；不得猜测 EXE/PATH。目标缺失/冲突不得阻断已经成功的运行态切换，而是在终态返回 typed launch warning。切换完成后只允许通过 Windows 原生应用激活接口启动并重新验证同一身份，不得执行 PATH 中同名程序或前端传入的路径/命令。
...
- v0.3 `SessionReferenceGraph` 的在线结果只能用于 Shadow 分类和报告：每个 runtime/backup SQLite 必须分别使用 Online Backup + `quick_check`，runtime 引用与备份库存必须分开计数；跨库快照天然非原子，禁止把“当前扫描零引用”当成删除授权。在线 GC 在 v0.3.0 固定为 scan-only。成功切换、迁移、冲突处理、待恢复导入和会话合并只向 single-flight/coalesced 调度器排队新 Shadow；普通 route 成功不得直接排队 provider GC。自动清理开启时，只有迁移已提交、scan identity 新鲜、零未完成操作且实时 writer inventory 全部关闭，调度器才可自动调用与手工入口相同的离线执行器。自动 GC/recovery/retention 的轮询必须先检查 generation 与 writer，再尝试 mutation guard，并在持锁后重新检查两者；不得持有全局 mutation guard 等待用户关闭 Codex/ChatGPT。writer 或 generation 窗口未形成时必须释放/不取得锁、零删除并保留最终校准请求；候选证明失败必须停止并报告。关闭自动清理后继续扫描/报告但不执行 provider 副本删除。任何正文删除、canonical 切换或数据库迁移在最终写入前都必须重新验证全局 runtime 引用、完整 semantic containment、完整文件 hash/marker、文件身份/句柄和未变化状态。
...
- route view state 必须保存 Account configured/effective SQLite home、严格受管的 Relay sibling path 和可空的 provider-normalized logical state digest。完全新建的 Codex Home 可以以 `lastCommonStateSha256 = null` 建立空 Relay 视图状态和受管空目录，但不得伪造 `state_5.sqlite`；Relay 首次自然创建数据库后，返回 Account 时再通过现有 Online Backup/provider normalization 路径物化 Account 视图。bootstrap 或后续配置失败必须删除本次创建的状态、空目录和缺失前不存在的 config，恢复精确缺失状态。已有共同基线时，覆盖 inactive `state_5.sqlite` 前必须证明它仍等于上次共同基线；缺失、外部改写、崩溃残留或 digest drift 都不得用“active 看起来较新”猜测覆盖。普通切换不做数据修复，交给迁移/合并账本流程。
- 首屏运行态和手机连续性只读初始化必须把尚未生成的 canonical `state_5.sqlite` 视为合法空状态：返回零队列/零会话且不得创建数据库。数据库路径已存在但不是 regular file、metadata 不可读或 SQLite 已损坏时仍必须返回脱敏错误并 fail closed；不得把所有读取失败都吞成空列表。
...
- 成功回执必须包含可关联的 operation ID 和 typed ChatGPT launch result。请求端切换的 progress/失败终态还必须携带稳定的 `RuntimeSwitchFailureReason`（包括官方认证缺失、认证状态损坏、配置不可用、会话视图不可用、独立 writer 活跃、mutation busy、进程关闭失败和 route verification 失败），UI 依据 reason 映射可执行中文指导，后端原始 message 只作次级细节，不得反解析。回执必须披露 route changed、auth 原字节或缺失状态 preserved、`chatProcessStateRepaired`、session-view status 与 `routeProvenance` typed terminal；ChatGPT 启动失败另携带 `ChatGptLaunchFailureReason`，缺失/歧义目标不提供无效自动重试，激活/验证失败可原位重试。为旧客户端兼容保留的 `relayValidation` 只能是 `skipped` / `notApplicable`，不得驱动网络请求或产品状态。不得保留恒空 backup/toShared/fromShared/checkpoint 字段或渲染“0 会话已同步”。高风险长流程必须通过后端 `Channel` 上报真实阶段和 typed 终态；后端原始 phase/timestamp 不得丢失，UI 可聚合为不超过 6 个用户步骤。ChatGPT launch 必须排在 route exact、provenance ready 与 durable terminal 之后；provenance failed/unknown 不得提供普通启动重试。
...
- `switch_runtime` 包含本地目标校验、进程等待、SQLite 会话视图准备和配置 I/O，必须放入 Tauri blocking worker；不得包含 Relay 网络探测。请求路由不得混入 scoped backup、全量 JSONL 扫描、provider materializer 或 GC。会话视图准备必须先于 config write：已有数据库时 Relay/Account 验证 provider-normalized state view 与共享全局库 identity；完全新建 Home 走无 fake DB 的 bootstrap。准备失败属于写入前失败并保留当前 route，bootstrap 失败还要清除本次创建的状态/空目录。进度通过 `Channel<RuntimeSwitchProgress>` 从真实后端阶段产生，并明确区分写入前失败、配置已回滚和回滚失败，同时携带稳定 typed failure reason。
...
- 前端生产验证必须覆盖 1200×820、900×640、390×844，逐档检查无横向溢出、主导航/关键按钮 hit-test、存储页 `aria-current` 往返、reduced-motion、中文与 console error；packed EXE 必须在隔离 `APPDATA`/`CODEX_HOME` 且无 Vite listener 时真实启动并 graceful close。每个发布候选还必须分别验证 initialized Home 与真正 fresh Home；fresh Home 要求 auth/config/state DB 保持缺失、运行态为 `unknown`、连续性队列为空。首轮连续性初始化占用 mutation guard 时全部写操作必须保持禁用，settle 后恢复；CDP/updater 验证必须等待 Tauri invoke bridge、连续性域与完整页面就绪后再调用命令，并对导航期间 execution-context 更换做有界重试。
