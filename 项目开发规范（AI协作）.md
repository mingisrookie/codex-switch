...
Relay 会话可见性必须通过独立 SQLite 视图保证：切入 Relay 前对 Account 四个受管数据库使用 SQLite Online Backup，只在 `%APPDATA%\codex-switch\relay-sqlite` 副本归一 thread provider；不得批量改写 Account 原库或原 JSONL。Relay route 必须同时写入受管 `sqlite_home`、`experimental_bearer_token`、`requires_openai_auth = true` 与 `supports_websockets = true`；`requires_openai_auth` 只维持 Desktop 官方账户识别，不替代 Relay 请求凭据。
