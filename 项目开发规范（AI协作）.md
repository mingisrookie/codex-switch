...
- 请求路由切换是上面快照合同的显式窄例外：`validate official auth -> optional relay access verification -> close -> replan config -> repair known process state -> atomic config apply -> verify auth bytes + route -> persist route terminal`。路由 mutation 不得扫描/同步会话、创建 checkpoint、执行 provider slot GC 或写 `auth.json`；配置写后的失败只允许回滚原始 `config.toml`。只有 Account 路由 terminal 已持久成功后，才可进入手机连续性安全发布准备，且连续性失败不得回滚请求端。
...
- v0.2.0 风格的 current `Runtime` + shared `Sessions` 全会话自动 checkpoint 禁止重新引入；请求路由和手机连续性发布都不得创建全池 snapshot。手机连续性只能为 cutover 后 eligible ID 创建 immutable rollout/marker 并在单库事务中更新活动行；不能退化为 current/shared 全量对账。会话/备份长期增长只能来自成功 successor、用户显式单条/完全同步、管理、恢复、Full 或安全保留项。
...
Relay 连接必须同时在前端做即时体验校验、在后端做权威校验；只接受无内嵌凭据/query/fragment 的 HTTPS Base URL，HTTP 仅允许 loopback。API Key 只能通过 password 表单进入，首次必填；后续只有规范化 URL 的 origin 不变时才允许空值保留旧密文，scheme/host/port 改变必须输入新 Key。Key 不得回填或回显。槽位中只存 DPAPI 密文；受管 Relay provider ID 必须由单一常量定义，当前只接受 `openai_custom`。激活 Relay 时允许且仅允许把解密值写入 live `config.toml` 的受管 `experimental_bearer_token`，切回 Account 必须按同一来源删除整个受管 provider 表。任何 TOML 解析错误、日志、回执或 UI 都不得包含 Key。Relay 首次切换或地址/凭据变化后必须让用户选择验证或直接切换；直接路径不得联网。连通性验证不得跟随重定向、不得输出响应正文或 Key，只以 2xx 判断 URL/TLS/HTTP/鉴权，不读取、推荐或判断模型列表。运行态 `exact` 判定必须比较所有影响请求路由的 provider 字段，包括 Base URL 与 bearer。

手机连续性必须使用独立版本化 cutover/queue；首次初始化只读取 SQLite thread ID，不扫描或上传旧 JSONL。队列只保存 thread ID、source fingerprint、typed 状态和脱敏失败分类，不保存正文、路径内容或凭据。自动领取只允许 cutover 后全新、未归档、受管 Relay provider 的会话；单批最多 8 thread/8 MiB，切回 Account 最多 4 批且总预算 30 秒，仍 deferred 必须保持 Relay route、不得先切回再隐藏会话。发布必须在 ChatGPT 关闭且 Account route config 尚未写入时完成，用 immutable/no-clobber provider successor、canonical containment、SQLite transaction/`PRAGMA quick_check` 和首条 metadata 复核。unsupported local fields 只能在 successor 中替换为不含本地细节的占位，原 source bytes 必须不变；分叉必须保留多个 successor 并返回 conflict，禁止 last-write-wins。没有手机侧证据时 UI 只能写“本机 Remote 已发布”或“已提交到手机同步”。

Relay 会话可见性必须通过独立 SQLite 视图保证：切入 Relay 前对 Account 四个受管数据库使用 SQLite Online Backup，只在 `%APPDATA%\codex-switch\relay-sqlite` 副本归一 thread provider；不得批量改写 Account 原库或原 JSONL。Relay route 必须同时写入受管 `sqlite_home`、`experimental_bearer_token` 与 `requires_openai_auth = true`；后者只维持 Desktop 官方账户识别，不替代 Relay 请求凭据。
...
- `switch_runtime` 包含网络验证、进程等待、SQLite 会话视图准备和配置 I/O，必须放入 Tauri blocking worker；请求路由不得混入 scoped backup、全量 JSONL 扫描或 provider GC。会话视图准备必须先于 config write：Relay 使用 Online Backup 副本，Account 使用有界 cutover 队列；准备失败属于写入前失败并保留当前 route。进度通过 `Channel<RuntimeSwitchProgress>` 从真实后端阶段产生，并明确区分写入前失败、配置已回滚和回滚失败。
