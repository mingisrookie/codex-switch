...
- 需要整文件替换的配置、备份和可写 index 统一走 `file_ops` 同目录完整临时文件 + sync + 原子替换，不得直接覆盖 live 文件。会话 source 在执行期必须校验完整尾行、session ID、长度和 SHA-256 前后稳定，只能做有界重试；Create 使用 provider 预归一的 hard-link no-clobber 发布；允许替换时，严格扩展和 Divergent 都必须按稳定 source hash 生成并发布完整 imported JSONL，旧目标 bytes/hash/mtime 不变。关闭态切换必须同时校验 SQLite 与首条 JSONL 的 provider；不匹配或 legacy imported 文件名不可被 Remote 枚举时，发布完整、provider 已归一且文件名符合 `rollout-<timestamp>-<thread-id>.jsonl` 的新 rollout，再推进活动引用；相同内容/provider 必须幂等复用。`SessionSyncPolicy.existing_rollout_path = ExistingRolloutPathPolicy::PreserveExisting` 仅允许用于 hot shared→current：检测到既有 current thread 后必须在 `existing_thread_rollout_path` / `copy_rollout_file` 前直接 duplicate+continue，保持零 target candidate I/O、零 orphan、`copied_session_files = 0`，不得改变既有 `rollout_path`、provider、title 或旧 writer 可见性；新 thread 仍在 SQLite 事务中插入。current→shared 与关闭态必须使用 `ExistingRolloutPathPolicy::SelectMostComplete`。hot current index 必须 `Skip`；closed/app-owned shared index 生成完整 merged bytes 后同目录 `atomic_write`；`Deny` 只校验且零写入。
...
- 会话 plan 不能作为执行授权：source 必须按完整尾行/session ID/len/hash 重新冻结，Create/Import/provider rollout 抢先出现时重新比较或 fail closed。允许替换时，严格 extension 与 Divergent 都发布按稳定 source hash 命名的完整 imported JSONL；旧文件不能修改。关闭态 provider rollout 必须使用 Remote 可识别的官方文件名形态，只有完整文件发布并复核 provider 后才能更新 SQLite。活动路径选择必须与文件发布解耦：hot shared→current 的 existing + `PreserveExisting` 不是文件 plan，必须在 target rollout path 查询和 `copy_rollout_file` 前直接跳过；current→shared/关闭态的 `SelectMostComplete` 才能在发布后选择更完整或 provider 已匹配的文件；`Unchanged` 在终态返回前必须复检 source version 与 live relation。index 必须显式选择 hot `Skip`、closed/app-owned `MergeAtomic` 或零写 `Deny`。provider 归一发生在完整新文件发布前，不能发布后再改写 live JSONL。
...
- 会话同步测试必须覆盖 source 尾行/session ID/len/hash 漂移与 bounded retry、抢先 Create、严格扩展与 Divergent 的稳定 hash-named 完整 import、旧短文件 bytes/hash/mtime 不变。必须分别锁定：hot shared→current 的既有 thread 在 target path 查询/复制前直接跳过、`copied_session_files = 0`、候选文件不存在、活动 `rollout_path`/provider/title 保留且旧 writer 同步后尾部继续可见；hot 新 thread 正常插入；current→shared 与关闭态 `SelectMostComplete` 可推进更完整引用。关闭态还必须覆盖 JSONL/SQLite provider 不一致、legacy imported 文件名、Remote 可识别的新 rollout、原文件不变和相同内容/provider 重复切换零新增。index 必须覆盖 hot current `Skip`、closed/app-owned shared 的完整 merged bytes 原子替换、`Deny` 零写和 `Unchanged` 终前复检；不得再把原地修改已有 JSONL/index 作为生产安全合同或测试要求。
...
- 会话性能回归必须使用临时 Home 证明关闭态 provider 切换保留原 JSONL 的 bytes/hash/mtime，首次只发布每个会话一个 Remote 可识别的 provider rollout，SQLite/活动 JSONL provider 一致，相同内容/provider 重复切换零新增；不得把合成代理误述为真实 ChatGPT 重索引 wall-clock。
